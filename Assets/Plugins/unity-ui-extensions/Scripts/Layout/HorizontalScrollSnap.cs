...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : ScrollSnapBase, IEndDragHandler
    {
        ...
        public void ChangePageOrder(int indexFrom, int indexTo)
        {
            if (indexFrom < 0 || indexFrom > _screensContainer.childCount ||
                indexTo < 0 || indexTo > _screensContainer.childCount)
            {
                return;
            }

            _scroll_rect.horizontalNormalizedPosition = 0;
            _screensContainer.GetChild(indexFrom).SetSiblingIndex(indexTo);
            InitialiseChildObjectsFromScene();
            DistributePages();
            if (MaskArea) UpdateVisible();
